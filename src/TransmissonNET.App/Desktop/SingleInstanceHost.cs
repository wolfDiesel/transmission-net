...
internal sealed class SingleInstanceHost : IAsyncDisposable
{
    ...
    {
        var custom = Environment.GetEnvironmentVariable("TRANSMISSIONNET_INSTANCE_SOCKET");
        if (!string.IsNullOrWhiteSpace(custom))
            return custom;

        var runtimeDir = Environment.GetEnvironmentVariable("XDG_RUNTIME_DIR");
        var baseDir = !string.IsNullOrWhiteSpace(runtimeDir) ? runtimeDir : Path.GetTempPath();
        return Path.Combine(baseDir, "transmission-net.sock");
    }
}
