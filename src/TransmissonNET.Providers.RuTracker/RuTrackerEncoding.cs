...
internal static class RuTrackerEncoding
{
    ...
    public static Encoding Resolve(string? charset)
    {
        ...
        if (name.Equals("windows-1251", StringComparison.OrdinalIgnoreCase)
            || name.Equals("cp1251", StringComparison.OrdinalIgnoreCase)
            || name.Equals("cp-1251", StringComparison.OrdinalIgnoreCase)
            || name.Equals("1251", StringComparison.OrdinalIgnoreCase))
            return Windows1251;
    ...
    public static ByteArrayContent CreateFormUrlEncoded(IEnumerable<KeyValuePair<string, string>> fields)
    {
        ...
        var content = new ByteArrayContent(Windows1251.GetBytes(body));
...
internal static class RuTrackerLog
{
    public static void Info(string message) =>
        Console.Error.WriteLine($"[RuTracker] {message}");

    public static void Error(string message, Exception? ex = null)
    {
        Console.Error.WriteLine($"[RuTracker] ERROR: {message}");
        if (ex is not null)
